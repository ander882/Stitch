...
function Script:main() {
  ...
  IO:log "Run as : $USER@$HOSTNAME"


  # Os:require "awk"
  Os:require "montage" "imagemagick" 

  msize=$(Str:lower "$msize")

  ########
  ##  Check the matrix size
  ########
  #TIP: [cols]x[rows] mean the number of colums/rows in the stithed together image
  #TIP:> use «$script_prefix 3x1» to make 3 horizontally stitched images ...
  #TIP:> use «$script_prefix 1x3» to make 3 vertically stitched images ...
  #TIP:> from a 1x2 (vertical) or 2x1 (horizontal) up to a 5x5 can be created
  [[ ${#msize} -ne 3 ]] && IO:die "Size of output matrix array incorrect"
  [[ ! $msize =~ [12345]x[12345] ]] && IO:die "Size of output matrix array incorrect"
  [[ $msize == 1x1 ]] && IO:die "Size of output matrix array incorrect"

  mvert=${msize:0:1}
  mhori=${msize:2:1}
  matrix_size=$((${mvert}*${mhori}))
  IO:log "size of the matrix will be: ${mvert}x${mhori}"
  ...
  #TIP: Images will normally rotate to the left and up
  #TIP:> use the -r option to have images rotate to the right and down
...
function IO:die() {
  ...
  IO:log   "${txtError}${char_fail} $script_basename${txtReset}: $*" >&2
  Os:beep
  Script:exit
}
...
function IO:success() {
  ...
  IO:log "${txtInfo}${char_succes}${txtReset}  ${txtBold}$*${txtReset}"
}

function IO:announce() {
  IO:print "${txtInfo}${char_wait}${txtReset}  ${txtItalic}$*${txtReset}"
  IO:log "${txtInfo}${char_wait}${txtReset}  ${txtItalic}$*${txtReset}"
  sleep 1
}
